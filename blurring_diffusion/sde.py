...
class VPSDE(SDE):
  def __init__(self, beta_min=0.1, beta_max=20, N=1000):
    ...
    self.alphas = torch.sqrt(1. - self.discrete_betas ** 2)
  ...
  def frequency_scaling(self, eigenvalue, t, sigma, min_scale = 0.001, sigma_blur_max = 1):
    ...
    eigenvalue = torch.diag_embed(eigenvalue)
    z = torch.randn_like(eigenvalue)
    scaling = torch.exp(- (eigenvalue + sigma[:, None, None] * z / 10) * t[:, None, None]) * (1 - min_scale)
    return scaling #(bs, max_node_num, max_node_num)
    
  def noise_scheduling(self, t, logsnr_min = -3, logsnr_max = 3):
    ...
    # logsnr = torch.tan(limit_min * t + limit_max) * 2.5
    # logsnr_flip = torch.tan(limit_min * (self.T - t) + limit_max) * 2.5
  ...
  def alpha_sigma(self, eigenvalue, t):
    ...
    freq_scaling = self.frequency_scaling(eigenvalue, t, sigma)
    
    alpha = a[:, None, None] * freq_scaling #(bs, max_node_num)
  ...
  def marginal_prob(self, x, t, eigenvalue, eigenvector):
    alpha, std = self.alpha_sigma(eigenvalue, t)

    eigvec_T = eigenvector.transpose(-1, -2)
    L = torch.matmul(eigenvector, (alpha * eigvec_T))
    mean = torch.matmul(L, x)
