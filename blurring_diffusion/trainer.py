...
class Trainer(object):
    ...
    def train(self, ts):
        ...
        print('\033[91m' + f'{self.config.ts}' + '\033[0m')
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            for _, train_b in enumerate(self.train_loader):
                ...
                x, adj, label, eigenvalue, eigenvector = load_batch(train_b, self.device) 
                # std, mean = torch.std_mean(adj)
                # adj = (adj - mean) / std
        
                loss_subject = (x, adj)
                eig_subject = (eigenvalue, eigenvector)
       
                loss_x, loss_adj = self.loss_fn(self.model_x, self.model_adj, *loss_subject, *eig_subject)
                loss_x.backward()
                loss_adj.backward()
            ...
            for _, test_b in enumerate(self.test_loader):   
                ...
                x, adj, label, eigenvalue, eigenvector = load_batch(test_b, self.device)
                # adj = (adj - mean) / std
                loss_subject = (x, adj)
                eig_subject = (eigenvalue, eigenvector)
