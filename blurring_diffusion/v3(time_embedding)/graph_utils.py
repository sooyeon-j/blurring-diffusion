...
def init_flags(graph_list, config, batch_size=None):
    ...
    graph_tensor = graphs_to_tensor(graph_list, max_node_num, compute_L=False)
...
def graphs_to_tensor(graph_list, max_node_num, compute_L = True):
    adjs_list = []
    eigval_list = []
    eigvec_list = []
    ...
    return (adjs_tensor, eigval_tensor, eigvec_tensor) if compute_L else adjs_tensor
...
def compute_laplacian(x): 
    ...
    diag[range(x.shape[1]), range(x.shape[1])] = x.sum(axis = -1)
    
    diag_inv_sqrt = np.linalg.inv(diag)
    
    laplacian = diag - x
    
    normalized = np.dot(np.sqrt(diag_inv_sqrt), np.dot(laplacian, np.sqrt(diag_inv_sqrt)))
