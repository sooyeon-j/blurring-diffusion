...
def backward(sde_x, sde_adj, shape_x, shape_adj, continuous = True, eps = 1e-3, device = 'cuda'):
  def denoise(model_x, model_adj, random_x, random_adj, init_flags):
    score_fn_x = get_score_fn(sde_x, model_x, train=False, continuous=continuous)
    score_fn_adj = get_score_fn(sde_adj, model_adj, train=False, continuous=continuous)
    
    with torch.no_grad():
      # x = random_x.to(device)
      # adj = random_adj.to(device)
      x = sde_x.prior_sampling(shape_x).to(device)
      adj = sde_adj.prior_sampling_sym(shape_adj).to(device)
      flags = init_flags
      x = mask_x(x, flags)
      ...
      for i in trange(0, diff_steps, desc = '[Sampling]', position = 1, leave=False):
        ...
        # score_x = score_fn_x(x, adj, flags, vec_t)
        # x_mean = score_x + x
        # x = x_mean + 0.1 * noise_x
        
        # score_adj = score_fn_adj(_x, adj, flags, vec_t)
        # adj_mean = score_adj + adj
        # adj = adj_mean + 0.1 * noise_adj
        
        alpha_s, sigma_s = sde_x.alpha_sigma(adj.shape[-1], vec_t_minus_1)
        alpha_t, sigma_t = sde_x.alpha_sigma(adj.shape[-1], vec_t)
        
        x_eps_hat = score_fn_x(x, adj, flags, vec_t)
        
        x = compute_reverse(x, alpha_s, sigma_s, alpha_t, sigma_t, x_eps_hat)
        
        adj_eps_hat = score_fn_adj(_x, adj, flags, vec_t)
        
        adj = compute_reverse(adj, alpha_s, sigma_s, alpha_t, sigma_t, adj_eps_hat)
  ...
  def compute_reverse(x, alpha_s, sigma_s, alpha_t, sigma_t, eps_hat):
    ...
    coeff1 = torch.clip(sigma2_denoise * alpha_ts / sigma_ts2, max = 1e-2)
    coeff2 = torch.clip(sigma2_denoise * alpha_st / (sigma_s ** 2)[:, None, None], max = 1e-2)

    # coeff1 = sigma2_denoise * alpha_ts / (sigma_ts2 + delta)
    # coeff2 = sigma2_denoise * alpha_st / torch.clip((sigma_s ** 2)[:, None, None], min = delta)
    
    u_t = dct.dct(x)
    term1 = torch.clip(dct.idct(torch.matmul(coeff1 , u_t)), max = 1e-2)
    term2 = torch.clip(dct.idct(torch.matmul(coeff2, u_t - (sigma_t[:, None, None] * dct.dct(eps_hat)))), max = 1e-2)
    mu_denoise = term1 + term2
    
    eps = torch.randn_like(mu_denoise)
    
    return mu_denoise + dct.idct(torch.matmul(torch.sqrt(sigma2_denoise), eps))
