...
def get_sde_loss_fn(sde_x, sde_adj, train=True, reduce_mean=False, continuous=True, 
  ...
  def loss_fn(model_x, model_adj, x, adj, eigenvalue, eigenvector): # model_x = ScoreNetworkX, model_adj = ScoreNetworkA, x = node in train data, adj = edge in train data
    ...
    t = torch.randint(0, sde_adj.step, (adj.shape[0],), device=adj.device) / sde_adj.step 
    #t = torch.rand(adj.shape[0], device=adj.device) * (sde_adj.T - eps) + eps
    flags = node_flags(adj)
    
    eigenvalue = eigenvalue.expand(adj.shape).to(adj.device)
    ...
    #   losses_x = torch.square(torch.matmul(std_x, score_x) + z_x)
    #   losses_x = reduce_op(losses_x.reshape(losses_x.shape[0], -1), dim=-1)

    #   losses_adj = torch.square(torch.matmul(std_adj, score_adj) + z_adj)
    #   losses_adj = reduce_op(losses_adj.reshape(losses_adj.shape[0], -1), dim=-1)

    z_x = gen_noise(x, flags, sym=False)
    mean_x, std_x = sde_x.marginal_prob(x, eigenvalue, eigenvector, t)
    # mean_x_ = sde_x.marginal_prob(x, eigenvalue, eigenvector, t - (1 / sde_x.step))[0]
    perturbed_x = mean_x + std_x * z_x
    perturbed_x = mask_x(perturbed_x, flags)

    z_adj = gen_noise(adj, flags, sym=True) 
    mean_adj, std_adj = sde_adj.marginal_prob(adj, eigenvalue, eigenvector, t)
    # mean_adj_ = sde_adj.marginal_prob(adj, eigenvalue, eigenvector, t - (1 / sde_adj.step))[0]
    ...
    if not likelihood_weighting:
      ...
      # losses_x = torch.square((perturbed_x + score_x) - mean_x_)
      losses_x = reduce_op(losses_x.reshape(losses_x.shape[0], -1), dim=-1)

      losses_adj = torch.square(score_adj - z_adj)
      # losses_adj = torch.square((perturbed_adj + score_adj) - mean_adj_)
      losses_adj = reduce_op(losses_adj.reshape(losses_adj.shape[0], -1), dim=-1)
