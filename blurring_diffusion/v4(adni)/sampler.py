...
from utils.loader import load_ckpt, load_data, load_seed, load_device, load_model_from_ckpt, \
                         load_ema_from_ckpt, load_sampling_fn, load_eval_settings, load_random_graph
...
class Sampler(object):
    ...
    def sample(self):
        ...
        self.train_graph_list, self.test_graph_list, self.eigenvalue, self.eigenvector = load_data(self.configt, get_graph_list= True)
        ...
        # self.random_x, self.random_adjs = load_random_graph(self.config)
...
class Sampler_mol(object):
    ...
    def sample(self):
        ...
        self.ckpt_dict = load_ckpt(self.config, self.device)
        self.configt = self.ckpt_dict['config']
        self.eigval = self.ckpt_dict['eigenvalues']
        self.eigvec = self.ckpt_dict['eigenvectors']
        ...
        x, adj, _ = self.sampling_fn(self.model_x, self.model_adj, self.init_flags, self.eigval, self.eigvec)
